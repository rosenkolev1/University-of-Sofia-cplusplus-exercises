...
const mstring MBigNumberEquation::EQUATION_INVALID_EXCEPTION = "Invalid Equation!";
const mstring MBigNumberEquation::EQUATION__SOLVE_TOOCOMPLEX_EXCEPTION = "Equation is too complex for me to solve";
const mstring MBigNumberEquation::EQUATION__SOLVE_TIEDTODIVISION_EXCEPTION = "The unknown is tied to division, so I can't solve this equation, sorry!";
const mstring MBigNumberEquation::EQUATION__SOLVE_NOTWHOLE_EXCEPTION = "Not whole number solution";
const mstring MBigNumberEquation::EQUATION__SOLVE_EVERYANSWER_EXCEPTION = "Every x is an answer to the equation";
const mstring MBigNumberEquation::EQUATION__SOLVE_NOANSWER_EXCEPTION = "No answer for this equation";
const mstring MBigNumberEquation::EQUATION_OPERATORS = "+-*/";

//0-49
int MBigNumberEquation::generateCountOfX(int seed)
{
	if (seed > 47) return 3;
...
MBigNumberEquation::MBigNumberEquation()
{
	this->equation = "x=0";
}

MBigNumberEquation::MBigNumberEquation(mstring equation)
{
...
bool MBigNumberEquation::isValidEquation(mstring equation) const
{
	...
	{
		MBigNumberExpression leftSide(sides[0]);
		MBigNumberExpression rightSide(sides[1]);
		...
		{
			if (MBigNumberExpression::EXPRESSION_DIVIDEBYZERO_EXCEPTION == e)
			{
				//Deallocate Dynamic Memory
				delete[] sides;
	...
	else
	{
		for (size_t i = 0; i < 2; i++)
		{
			...
			MBigNumberExpression sideExpression;
...
bool MBigNumberEquation::isValidEquation() const
{
	return this->isValidEquation(this->equation);
}

MBigNumber MBigNumberEquation::solveEquation(mstring equation)
{
	...
	MBigNumber testValue1(1);
	MBigNumber testValue2(2);
	MBigNumberExpression knownSideExpression(knownSide);

	mstring expressionTestValue1String = MStringManip::replaceFirst(unknownSide, "x", testValue1.getNumberRaw());
	MBigNumberExpression expressionTestValue1(expressionTestValue1String);

	mstring expressionTestValue2String = MStringManip::replaceFirst(unknownSide, "x", testValue2.getNumberRaw());
	MBigNumberExpression expressionTestValue2(expressionTestValue2String);

	MBigNumber testValue1Result = (expressionTestValue1 - knownSideExpression).evaluateExpression();
	if (testValue1Result.getSign() == -1) testValue1Result *= -1;
	MBigNumber testValue2Result = (expressionTestValue2 - knownSideExpression).evaluateExpression();
	if (testValue2Result.getSign() == -1) testValue2Result *= -1;

	MBigNumber deltaResult = testValue2Result - testValue1Result;
	...
	{
		MBigNumber decreaseOfX = testValue1Result / deltaResult;

		MBigNumber decreaseOfXLeftover = testValue1Result % deltaResult;
		...
		{
			MBigNumber potentialAnswer = (decreaseOfX - 1) * -1;
			...
			MBigNumberExpression expressionPotenialAnswer(expressionPotenialAnswerString);
			...
			MBigNumber potentialAnswerResult = (expressionPotenialAnswer - knownSideExpression).evaluateExpression();
...
MBigNumber MBigNumberEquation::solveEquation()
{
	return this->solveEquation(this->equation);
}

mstring MBigNumberEquation::generateFromTemplate(mstring expressionTemplate, size_t maxUnknown)
{
...
void MBigNumberEquation::generateEquation(mstring leftSideAllowedOperators, mstring rightSideAllowedOperators, size_t maxUnknownPerSide, size_t totalUnknown)
{
	...
	MBigNumberExpression expression;
...
void MBigNumberEquation::generateEquation(size_t maxUnknownPerSide, size_t totalUnknown)
{
	generateEquation(EQUATION_OPERATORS, EQUATION_OPERATORS, maxUnknownPerSide, totalUnknown);
}

mstring MBigNumberEquation::getEquation() const
{
	return this->equation;
}

void MBigNumberEquation::setEquation(mstring equation)
{
...
mstring MBigNumberEquation::getRightExpression() const
{
	size_t arraySize = 0;
...
mstring MBigNumberEquation::getLeftExpression() const
{
	size_t arraySize = 0;
	mstring* expressions = MStringManip::splitString(this->getEquation(), "=", arraySize);
...
mstring MBigNumberEquation::getEquationTemplate(mstring equation) const
{
...
mstring MBigNumberEquation::getEquationTemplate() const
{
	return this->getEquationTemplate(this->equation);
}

std::istream& operator>>(std::istream& is, MBigNumberEquation& equation)
{
...
std::ostream& operator<<(std::ostream& os, MBigNumberEquation& equation)
{
...
std::ifstream& operator>>(std::ifstream& is, MBigNumberEquation& equation)
{
...
std::ofstream& operator<<(std::ofstream& os, MBigNumberEquation& equation)
{
	os << equation.getEquation() << "\n";
	os << "The answer to the equation: ";
	try
